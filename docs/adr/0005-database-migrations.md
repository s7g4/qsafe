...
1. Define the initial tables in a tracked SQL file: [0001_init.sql](file:///c:/Users/Shaurya/OneDrive/Desktop/projects/qsafe/host-server/migrations/0001_init.sql).
2. Invoke `sqlx::migrate!().run(&pool).await?` inside `Database::new` during connection pool configuration.
3. Remove the inline `create_tables` logic and its explicit call in [main.rs](file:///c:/Users/Shaurya/OneDrive/Desktop/projects/qsafe/host-server/src/main.rs).
