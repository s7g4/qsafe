...
pub enum WSMessage {
    ...
    SendMessage { content: String, recipient_id: String },
    MessageReceived { message: String, sender_id: String },
    Error { message: String },
}

pub type ConnectedClients = Arc<Mutex<HashMap<String, futures_util::stream::SplitSink<WebSocket, Message>>>>;
        ...
        if let Message::Text(text) = msg {
            if let Ok(ws_msg) = serde_json::from_str::<WSMessage>(&text) {
                match ws_msg {
                    ...
                    WSMessage::SendMessage { content, recipient_id: _ } => {
