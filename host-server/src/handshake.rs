...
use crate::crypto::{CryptoEngine, HybridSharedSecret, KeyPair, Signature};
use crate::qrng::QRNG;
use crate::qkd::{QKDProtocol, DecoyCheck};
use blake3;
...
pub struct HandshakeInit {
    ...
    pub identity_sig: Signature, // Signed init message
}
...
impl Handshake {
    ...
    pub fn alice_initiate(&mut self, key_length: usize, decoy_fraction: f64) -> Result<(HandshakeInit, Vec<u8>), Box<dyn Error>> {
        ...
        let (decoy_indices, decoy_bases) = self.qrng.generate_decoy_bits(key_length * 2, decoy_fraction);
        let decoy_data = bincode::serialize(&(decoy_indices, &decoy_bases[..]))?;
        let decoy_commit = blake3::hash(&decoy_data).as_bytes().to_vec();

        // Sign init
        let init_msg = bincode::serialize(&(kyber_kp.public_key.clone(), x25519_kp.public_key.clone(), decoy_commit.clone()))?;
        let identity_kp = self.crypto.generate_ed25519_keypair()?; // Assume persistent identity
        let identity_sig = self.crypto.sign_ed25519(&identity_kp.secret_key, &init_msg)?;
        ...
        let private_data = bincode::serialize(&(kyber_kp.secret_key, x25519_kp.secret_key, identity_kp.secret_key, decoy_data))?;

        Ok((init, private_data))
    }

    /// Bob responds to init
    pub fn bob_respond(&mut self, init: &HandshakeInit, private_alice_data: Option<Vec<u8>>) -> Result<(HandshakeResponse, Vec<u8>), Box<dyn Error>> {
        ...
        let x25519_ss = self.crypto.x25519_shared_secret(&x25519_kp_bob.secret_key, &init.x25519_pub)?;
        ...
        let confirmation_hash = self.crypto.key_confirmation_mac(&session_key, b"bob-confirmation");

        // Sign response
        let resp_msg = bincode::serialize(&(kyber_ct.clone(), x25519_kp_bob.public_key.clone(), decoy_check.clone(), confirmation_hash.clone()))?;
        let identity_kp_bob = self.crypto.generate_ed25519_keypair()?;
        let identity_sig = self.crypto.sign_ed25519(&identity_kp_bob.secret_key, &resp_msg)?;
        ...
        // Store Bob's private
        let private_data = bincode::serialize(&(kyber_kp_bob.secret_key, x25519_kp_bob.secret_key, identity_kp_bob.secret_key, session_key))?;

        Ok((response, private_data))
    }

    /// Alice finalizes handshake
    pub fn alice_finalize(&mut self, init: &HandshakeInit, response: &HandshakeResponse, private_data: Vec<u8>) -> Result<HandshakeResult, Box<dyn Error>> {
        let (mut kyber_sk, mut x25519_sk, _, decoy_data): (Vec<u8>, Vec<u8>, Vec<u8>, Vec<u8>) = bincode::deserialize(&private_data)?;

        // Decapsulate Kyber
        let kyber_ss = self.crypto.kyber_decapsulate(&kyber_sk, &response.kyber_ct)?;

        // Compute X25519 shared (Alice's secret * Bob's pub)
        let x25519_ss = self.crypto.x25519_shared_secret(&x25519_sk, &response.x25519_pub)?;
        ...
        // Verify decoy commit
        let (decoy_indices, decoy_bases): (Vec<usize>, Vec<u8>) = bincode::deserialize(&decoy_data)?;
        ...
        if computed_commit != init.decoy_commit { // Assume init from context
        ...
        let expected_confirm = self.crypto.key_confirmation_mac(&session_key, b"alice-confirmation");
    ...
    pub fn bob_verify_confirmation(&self, alice_confirm: &[u8], session_key: &[u8]) -> bool {
        let expected = self.crypto.key_confirmation_mac(session_key, b"alice-confirmation");
    ...
    fn perform_decoy_check(&mut self, key_length: usize, decoy_fraction: f64) -> DecoyCheck {
        ...
        let qkd_key = self.qkd.bb84_with_decoy_checks(key_length, decoy_fraction).unwrap();
        DecoyCheck {
