...
impl MessagingService {
    ...
    pub async fn establish_session(
        ...
        let (response, _bob_private) = self
            .handshake
            .bob_respond(&init, Some(alice_private.clone()))?;

        // Alice finalizes
        let result = self
            .handshake
            .alice_finalize(&init, &response, alice_private)?;
    ...
    pub async fn send_message(
        ...
        let (ciphertext, _nonce) =
            self.crypto
                .encrypt_aead(&session.shared_key, plaintext.as_bytes(), Some(&aad))?;
    ...
    pub async fn receive_messages(
        ...
        while let Ok(message) = self.message_receiver.try_recv() {
            ...
            if !self.crypto.verify_ed25519(
                &keypair.public_key,
                &crate::crypto::QSafeSignature {
            ...
            let plaintext = self.crypto.decrypt_aead(
                &session.shared_key,
                &message.content,
                &[0u8; 12],
                Some(&aad),
