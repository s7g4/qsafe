...
use pqcrypto_traits::kem::{PublicKey as KyberPublicKey, SecretKey as KyberSecretKey, SharedSecret as KyberSharedSecret, Ciphertext as KyberCiphertext};
use x25519_dalek::{PublicKey, EphemeralSecret};
use orion::aead::{seal, open, SecretKey as AeadSecretKey};
use blake3;
use hkdf::Hkdf;
use sha3::Sha3_256;
...
impl CryptoEngine {
    ...
    pub fn kyber_encapsulate(&mut self, public_key: &[u8]) -> Result<(Vec<u8>, Vec<u8>), Box<dyn std::error::Error>> {
    ...
    pub fn kyber_decapsulate(&mut self, secret_key: &[u8], ciphertext: &[u8]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    pub fn generate_x25519_keypair(&mut self) -> Result<KeyPair, Box<dyn std::error::Error>> {
        let secret = EphemeralSecret::new(&mut self.rng);
    ...
    pub fn x25519_shared_secret(&self, secret_key: &[u8], public_key: &[u8]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    pub fn hybrid_key_agreement(&self, kyber_ss: &[u8], x25519_ss: &[u8]) -> Result<HybridSharedSecret, Box<dyn std::error::Error>> {
        ...
        hk.expand(b"qsafe-session-key", &mut session_key).map_err(Into::<Box<dyn std::error::Error>>::into)?;
    ...
    pub fn sign_ed25519(&self, secret_key: &[u8], message: &[u8]) -> Result<QSafeSignature, Box<dyn std::error::Error>> {
    ...
    pub fn verify_ed25519(&self, public_key: &[u8], signature: &QSafeSignature) -> Result<bool, Box<dyn std::error::Error>> {
        let verifying_key = VerifyingKey::from_bytes(&public_key.try_into()?)?;
        let sig = Signature::from_bytes(&signature.signature.try_into()?)?;
        Ok(verifying_key.verify(&signature.message, &sig).is_ok())
    }

    /// Encrypt a message using ChaCha20-Poly1305 AEAD
    pub fn encrypt_aead(&mut self, key: &[u8], plaintext: &[u8], _aad: Option<&[u8]>) -> Result<(Vec<u8>, Vec<u8>), Box<dyn std::error::Error>> {
    ...
    pub fn decrypt_aead(&self, key: &[u8], ciphertext: &[u8], nonce: &[u8], _aad: Option<&[u8]>) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
        let secret_key = AeadSecretKey::from_slice(key)?;
    ...
    pub fn key_confirmation_mac(&self, key: &[u8], message: &[u8]) -> Vec<u8> {
        ...
        blake3::keyed_hash(key_hash.as_bytes(), message).as_bytes().to_vec()
    }
    ...
    pub fn sign(&mut self, secret_key: &[u8], message: &[u8]) -> Result<QSafeSignature, Box<dyn std::error::Error>> {
        self.sign_ed25519(secret_key, message)
    }

    pub fn verify(&mut self, public_key: &[u8], signature: &QSafeSignature) -> Result<bool, Box<dyn std::error::Error>> {
